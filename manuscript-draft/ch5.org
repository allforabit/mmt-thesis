...
#+LATEX: \end{footnotesize}
If the probability is set to ~1.0~ (which is the default) or fully saturated the
...
state machine/ (FSM) whose transitions are caused by user actions. The
...
To achieve this, the ~:note~ subscription is itself subscribed to
